...
sealed class AuthSessionState extends Equatable {
  ...
  /// Accounts made before the gate shipped never saw a verification step, so
  /// some of them hold addresses their owner cannot read. Gating those would
  /// lock the person out for good — the link can never arrive — so the cut-off
  ...
  static final DateTime verificationRequiredFrom = DateTime.utc(2026, 8, 28);
  ...
  /// The address on the session, or an empty string when there is none. Read
  /// by the verify screen, which has to name the inbox it is waiting on.
  ...
  bool get needsVerificationGate {
    ...
    // Google vouched for the address already and there is no password for a
    // link to protect, so a Google-only session is never gated even in the
    // rare case Firebase reports it unverified.
    if (!session.hasPassword) return false;
    // An account this build created is gated whatever the date says. The
    // cut-off below has to sit a week ahead of the build so a staged rollout
    // cannot catch an old-build sign-up, and that would otherwise leave a
    // week in which new sign-ups on this build walked straight past the gate.
    // See [LocalStore.markGatedSignup].
    if (LocalStore.instance.isGatedSignup(session.user.uid)) return true;
...
class AuthAuthenticatedSession extends AuthSessionState {
  ...
  /// Whether the account was created with an email and a password. Google
  /// accounts have no password of ours.
  ...
  @override
  List<Object?> get props => [user.uid, emailVerified, hasPassword];
}
