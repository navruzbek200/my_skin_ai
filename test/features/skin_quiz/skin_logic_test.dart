...
void main() {
  // Helper: builds a 10-element answer list with specific index overrides.
  List<dynamic> answers({Map<int, int> set = const {}}) {
    final list = List<dynamic>.filled(10, 0);
  ...
  group('threshold blocks', () {
    test('all-zero answers → no additional blocks', () {
      expect(SkinLogic.analyze(answers()).additionalBlocks, isEmpty);
    });
    ...
    test('below-threshold (idx-4 = 2) → no Bh block', () {
      final r = SkinLogic.analyze(answers(set: {4: 2}));
  ...
  group('edge cases', () {
    ...
    test('non-int q1 → treated as defaultVal=2 → Aralash', () {
      final r = SkinLogic.analyze(['oily', 0, 5, 0, 0, 0, 0, 0, 0, 0]);
      expect(r.skinType, 'Aralash');
    });

    test('out-of-range index reads → default 0, no extra blocks', () {
      // 3 answers — indices 3..9 out of range → all default 0 → no concern blocks
